...
import { ArrowDown, ArrowRight, ArrowUpRight, Play, Pause, RotateCcw, MoveDown, Sparkles, Send, Copy, Check, X } from 'lucide-react';
    ...
    <div className="ride-viewport">
      ...
      <main id="ride-content" className={`ride-copy ride-copy-stage-${stage}`} tabIndex={-1}>
        ...
        <div key={stage} className="ride-chapter">
          ...
          {stage===0&&<>
            <h1>Life’s a ride.<br/>Make it <em>wild.</em><span className="ride-spark">✳</span></h1>
          ...
          {stage===2&&<><h1>Up. Down.<br/>Still <em>wheeee!</em></h1><p className="ride-description">Bitcoin is the original roller coaster. $BRCG brings that energy to the Robinhood network. A community for everyone who enjoys the ride.</p><div className="ride-manifesto"><div><span>↗</span><p>GREEN DAY<strong>Hands up.</strong></p></div><div><span>↘</span><p>RED DAY<strong>Hands still up.</strong></p></div></div><span className="ride-small-note">NO PRICE PREDICTIONS. JUST ROLLER COASTER ENERGY.</span></>}
          {stage===3&&<><h1>Big thrills.<br/><em>Simple numbers.</em></h1><div className="ride-stats"><div><span>TOTAL SUPPLY</span><strong>420.69<span>B</span></strong></div><div><span>BUY / SELL TAX</span><strong>0<span>%</span></strong></div><div><span>PAIRED ASSET</span><b>Bitcoin ↗</b></div><div><span>NETWORK</span><b>Robinhood</b></div></div><p className="ride-small-note">PROJECT TOKENOMICS · FAIR LAUNCH VIA LONG.XYZ</p></>}
          {stage===4&&<><h1>Your seat.<br/><em>Your ride.</em></h1><p className="ride-description">The best part of the roller coaster?<br/>The people screaming next to you.</p><div className="ride-actions"><a className="ride-primary" href={TELEGRAM_URL} target="_blank" rel="noreferrer"><Send size={17}/> Join the community</a><a className="ride-x-button" href={X_URL} target="_blank" rel="noreferrer"><XIcon/> Follow on X</a></div><div className="ride-launch"><span>THE NEXT ADVENTURE</span><a href={LAUNCH_URL} target="_blank" rel="noreferrer">Explore long.xyz <ArrowUpRight size={17}/></a><p>Contract address: coming soon.</p></div><p className="ride-disclaimer">$BRCG is a meme token. No promises of returns. Not financial advice.</p></>}
        </div>
      </main>
      <div className="ride-stamp"><span>₿</span><p>POWERED BY<br/><strong>THE UPS & DOWNS</strong></p><Sparkles size={18}/></div>
      <div className="ride-scene-caption"><span className="ride-caption-dot"/><span>{['THE ADVENTURE STARTS HERE','CLIMBING INTO THE LORE','ENJOY EVERY TWIST','A LITTLE CLARITY AT THE TOP','THERE’S ALWAYS ANOTHER RIDE'][stage]}</span><span>↗</span></div>
      ...
      <div className="ride-bottom"><span>© {new Date().getFullYear()} $BRCG · BUILT FOR THE RIDE</span><button className="ride-motion" onClick={()=>{setReduced(!reduced);setAuto(false);}}>{reduced?'Motion: reduced':'Motion: full'}</button><a href="/classic">Classic website ↗</a></div>
